...
from helpers.generators import fit_ba
...
def _execute_one_graph(graph_dict):
    ...
    model_name = "BA circle"
    model_converter = lambda g: ("", fit_ba(g, fully_connected_start=False))
...
class GeneratorBACircle(AbstractStage):
    _stage = "2-features/ba-circle"

    def __init__(self, graph_dicts, cores=1, **kwargs):
        super(GeneratorBACircle, self).__init__()
...
def main():
    ...
    generator = GeneratorBACircle(graph_dicts, cores=args.cores)
    generator.execute()
