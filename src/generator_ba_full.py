...
def _execute_one_graph(graph_dict):
    ...
    model_name = "BA full"
    model_converter = lambda g: ("", fit_ba(g, fully_connected_start=True))
...
class GeneratorBAFull(AbstractStage):
    _stage = "2-features/ba-full"

    def __init__(self, graph_dicts, cores=1, **kwargs):
        super(GeneratorBAFull, self).__init__()
...
def main():
    ...
    generator = GeneratorBAFull(graph_dicts, cores=args.cores)
    generator.execute()
