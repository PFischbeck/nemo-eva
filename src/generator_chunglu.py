...
def _execute_one_graph(graph_dict):
    ...
    model_name = "chung-lu"
    model_converter = lambda g: ("", fit_chung_lu(g, connected=True))
...
class GeneratorChungLu(AbstractStage):
    _stage = "2-features/chung-lu"

    def __init__(self, graph_dicts, cores=1, **kwargs):
        super(GeneratorChungLu, self).__init__()
...
def main():
    ...
    generator = GeneratorChungLu(graph_dicts, cores=args.cores)
    generator.execute()
