...
def _execute_one_graph(graph_dict):
    ...
    model_name = "ER"
    model_converter = lambda g: ("", fit_er(g, connected=True))
...
class GeneratorER(AbstractStage):
    _stage = "2-features/er"

    def __init__(self, graph_dicts, cores=1, **kwargs):
        super(GeneratorER, self).__init__()
...
def main():
    ...
    generator = GeneratorER(graph_dicts, cores=args.cores)
    generator.execute()
