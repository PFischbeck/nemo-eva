...
def _execute_one_graph(parameters):
    ...
    try:
        info, model = generate_girg(dimension, n, m, cc, gamma, connected=True)
...
class GeneratorGirgSelf(AbstractStage):
    _stage = "2-features/girg-self"

    def __init__(self, parameters, cores=1, **kwargs):
        super(GeneratorGirgSelf, self).__init__()
...
def main():
    ...
    generator = GeneratorGirgSelf(parameters, cores=args.cores)
    generator.execute()
